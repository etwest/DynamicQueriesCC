...
SketchDelta const Sketch::get_delta(const vec_t update_idx) {
  ...
  for (unsigned i = 0; i < num_columns; ++i) {
    ...
    size_t bucket_id = i * bkt_per_col + depth;
    likely_if(depth < bkt_per_col) {
      delta.bucket_ids[i] = bucket_id;
    } else {
      delta.bucket_ids[i] = (size_t)-1;
    }
  }
...
void Sketch::apply_deterministic_delta(const SketchDelta delta) {
  // Update depth 0 bucket
  Bucket_Boruvka::update(buckets[num_buckets - 1], delta.update_idx, delta.checksum);
}
#endif
...
bool Sketch::empty() {
  return (buckets[num_buckets-1].alpha == 0 && buckets[num_buckets-1].gamma == 0);
}
