...
#define CANARY(X) ;
#define ENDPOINT_CANARY(X, src, dst) do {if (tier_num == 1 && (src == 7781 || dst == 7781)) {int canary_h; MPI_Comm_rank(MPI_COMM_WORLD, &canary_h); std::cout << __FILE__ << ":" << __LINE__ << " @ Tier " << canary_h-1 << " says " << X << " " << src << " " << dst << std::endl;}} while (false)
